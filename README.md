# NeetCode Solutions — @Srihari2222

> Synced automatically from [NeetCode.io](https://neetcode.io) · Repository: `neetcode-submissions-rkxdbkjd`
...
2. **Auto-commit** — every time you submit a solution on NeetCode, it is pushed here automatically (configurable by status, language, and difficulty filters).
...
Solutions are organized by problem ID. Each submission is stored as a separate file:
...
- **Status filter** — sync all submissions, accepted only, or wrong answers only
- **Language filter** — limit syncing to a specific language
- **Difficulty filter** — limit syncing to Easy, Medium, or Hard problems
- **Rename repository** — rename this repo or start fresh with a new one
- **Bulk Sync** — push all past solutions at once (rate-limited: weekly for Pro, monthly for Free)
